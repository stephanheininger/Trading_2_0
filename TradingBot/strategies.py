...
class Strategy:
    ...
    def parse_trades(self, price: float, size: float, timestamp_var: int):

        timestamp_diff = int(time.time() * 1000) - timestamp_var
        if timestamp_diff >= 2000:
            logger.warning("%s %s: %s milliseconds of difference between the current time and the trade time", self.exchange,
                           self.contract.symbol, timestamp_diff)

        last_candle = self.candles[-1]

        # Same Candle
        if timestamp_var < last_candle.timestamp + self.tf_equiv:
            ...
            for trade in self.trades:
                if trade.status == "open" and trade.entry_price is not None:
                    self.check_tp_sl(trade)    
            return "same_candle"

        # Missing Candle(s)
        elif timestamp_var >= last_candle.timestamp + 2 * self.tf_equiv:

            missing_candles = int((timestamp_var - last_candle.timestamp) / self.tf_equiv) - 1

            logger.info("%s missing %s candles for %s %s (%s %s)", self.exchange, missing_candles, self.contract.symbol,
                        self.tf, timestamp_var, last_candle.timestamp)
            ...
            new_ts = last_candle.timestamp + self.tf_equiv
            candle_info = {'ts': new_ts, 'open': price, 'high': price, 'low': price,
        ...
        # New Candle
        elif timestamp_var >= last_candle.timestamp + self.tf_equiv:
            new_ts = last_candle.timestamp + self.tf_equiv
            candle_info = {'ts': new_ts, 'open': price, 'high': price, 'low': price,
                           'close': price, 'volume': size}
            new_candle = Candle(candle_info, self.tf, "parse_trades")
            ...
            if order_status.status == "filled":
                for trade in self.trades:
                    if trade.entry_id == order_id:
                        ...
                        break
                return
        t = Timer(2.0, lambda: self.check_order_status(order_id))
        t.start()


    def open_position(self, signal_result: int):
        ...
        self.add_log(f"{position_side} signal on {self.contract.symbol} {self.tf}")

        order_status = self.client.place_order(self.contract, "MARKET", trade_size, order_side)

        if order_status is not None:
            self.add_log(f"{order_side.capitalize()} order placed on {self.exchange} | Status: {order_status.status}")
            ...
            if order_status.status == "filled":
                ...
                t = Timer(2.0, lambda: self.check_order_status(order_status.order_id))
                t.start()
            new_trade = Trade({"time": int(time.time() * 1000), "entry_price": avg_fill_price, "contract": self.contract, "strategy":  self.strat_name,
                               "side": position_side, "status": "open", "pnl": 0, "quantity": trade_size, "entry_id": order_status.order_id})
            self.trades.append(new_trade)

    def check_tp_sl(self, trade: Trade):
        tp_triggered = False
        sl_triggered = False
        ...
        if tp_triggered or sl_triggered:
            self.add_log(f"{'Stop loss' if sl_triggered else 'Take profit'} for {self.contract.symbol} {self.tf}")
            ...
            if order_status is not None:
                self.add_log(f"Exit order on {self.contract.symbol} {self.tf} placed succesfully")
                trade.status = "closed"
                self.ongoing_position = False
...
class TechnicalStrategy(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_pct: float, take_profit: float,
        ...
        self.rsi_length = other_params['rsi_length']
    ...
    def rsi(self):
        ...
        avg_gain = up.ewm(com=(self.rsi_length - 1), min_periods=self.rsi_length).mean()
        avg_loss = down.abs().ewm(com=(self.rsi_length - 1), min_periods=self.rsi_length).mean()
    ...
    def check_signal(self):
        macd_line, macd_signal = self.macd()
        rsi = self.rsi()
